...
class CommandParamsFactory extends Handler {
	...
	// used,
	// maximum 239 chars can be encoded in 239 bytes since each char takes 1
	// byte.
	//
	...
	static synchronized CommandParamsFactory getInstance(
	...
	static synchronized CommandParamsFactory getInstance(
			RilMessageDecoder caller, IccFileHandler fh) {
		if (sInstance != null) {
			return sInstance;
		}
	...
	private CommandParamsFactory(RilMessageDecoder paramRilMessageDecoder) {
		...
		if (ctlvs != null) {
			...
			if (ctlvCmdDet != null) {
				try {
					...
					CatLog.d(this,
							"processCommandDetails: Failed to procees command details e="
									+ e);
				}
			}
		}
	...
	private ResultCode setIcons(Object data) {
		...
		if (data == null) {
			if (mloadIcon) {
				...
				mloadIcon = false;
				/**
				 * In case of icon load fail consider the received proactive
				 * command as valid (sending RESULT OK)
				 */
		...
		case LOAD_MULTI_ICONS:
			...
			for (Bitmap icon : icons) {
				...
				if (icon == null && mloadIcon) {
					CatLog.d(this,
							"Optional Icon data is NULL while loading multi icons");
					mCmdParams.loadIconFailed = true;
				}
			}
			break;
	 ...
	 *            A tag to search for
	 * @param ctlvs
	 *            List of ComprehensionTlv objects used to search in
	 * 
	 ...
	 *            A tag to search for
	 * @param iter
	 *            Iterator for ComprehensionTlv objects used for search
	 * 
	 * @return A ComprehensionTlv object that has the tag value of {@code tag}.
	 *         If no object is found with the tag, null is returned.
	 */
	 ...
	 * 
	 * @param cmdDet
	 *            Command Details container object.
	 * @param ctlvs
	 *            List of ComprehensionTlv objects following Command Details
	...
	private boolean processDisplayText(CommandDetails cmdDet,
		...
		if (iconId != null) {
			...
			mIconLoader.loadIcon(iconId.recordNumber,
					this.obtainMessage(MSG_ID_LOAD_ICON_DONE));
			return true;
		}
		return false;
	}
	...
	private boolean processSetUpIdleModeText(CommandDetails cmdDet,
		...
		if (textMsg.text == null && iconId != null
	...
	private boolean processGetInput(CommandDetails cmdDet,
		...
		if (input.ucs2 && input.maxLen > MAX_UCS2_CHARS) {
			...
			CatLog.d(this, "GSM 7Bit Default: received maxLen = "
					+ input.maxLen + ", truncating to "
	...
	private boolean processSelectItem(CommandDetails cmdDet,
		...
		case LOAD_SINGLE_ICON:
			mloadIcon = true;
			mIconLoader.loadIcon(titleIconId.recordNumber,
					this.obtainMessage(MSG_ID_LOAD_ICON_DONE));
			break;
		case LOAD_MULTI_ICONS:
			...
			mloadIcon = true;
			mIconLoader.loadIcons(recordNumbers,
					this.obtainMessage(MSG_ID_LOAD_ICON_DONE));
			break;
		}
		return true;
	 ...
	 * 
	 * @param cmdDet
	 *            Command Details object retrieved.
	 * @param ctlvs
	 *            List of ComprehensionTlv objects following Command Details
	 *            object and Device Identities object within the proactive
	 *            command
	 * @return false. This function always returns false meaning that the
	 *         command processing is not pending and additional asynchronous
	...
	private boolean processSetUpEventList(CommandDetails cmdDet,
		...
		if (ctlv != null) {
			try {
				...
				CatLog.d(this,
						" IndexOutofBoundException in processSetUpEventList");
			}
		}
		return false;
	}
	...
	private boolean processPlayTone(CommandDetails cmdDet,
		...
		textMsg.responseNeeded = false;
		mCmdParams = new PlayToneParams(cmdDet, textMsg, tone, duration,
				vibrate);
	...
	private boolean processSetupCall(CommandDetails cmdDet,
		...
		if (confirmIconId != null || callIconId != null) {
			...
			recordNumbers[0] = confirmIconId != null ? confirmIconId.recordNumber
					: -1;
	...
	private boolean processProvideLocalInfo(CommandDetails cmdDet,
			List<ComprehensionTlv> ctlvs) throws ResultException {
		...
		case LANGUAGE_SETTING:
			...
			break;
		default:
			CatLog.d(this, "PLI[" + cmdDet.commandQualifier
	...
	private boolean processBIPClient(CommandDetails cmdDet,
			List<ComprehensionTlv> ctlvs) throws ResultException {
		AppInterface.CommandType commandType = AppInterface.CommandType
				.fromInt(cmdDet.typeOfCommand);
		if (commandType != null) {
