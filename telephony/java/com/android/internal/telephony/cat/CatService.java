...
public class CatService extends Handler implements AppInterface {
	...
	static final int MSG_INIT_RADIO_ON = 40;
	...
	private UiccController mUiccManager;
	...
	public CatService(CommandsInterface ci, Context context, int slotId) {
		...
		this.mSlotId = slotId;
		this.mHandlerThread = new HandlerThread("Cat Telephony service"
				+ slotId);
		this.mHandlerThread.start();
		this.mMsgDecoder = RilMessageDecoder.getInstance();
		...
		this.mCmdIf.registerForOn(this, MSG_INIT_RADIO_ON, null);
		CatLog.d(this, "CatService:registerForIccStatusChanged");
		this.mUiccManager = UiccController.getInstance();
		if (this.mUiccManager != null) {
			this.mUiccManager.registerForIccChanged(this, MSG_ID_ICC_RECORDS_LOADED, null);
			this.mStkAppInstalled = isStkAppInstalled();
			CatLog.d(this, "Running CAT service. STK app installed:"
					+ this.mStkAppInstalled + "slot:" + this.mSlotId);
		} else {
	...
	private CatService(CommandsInterface ci, UiccCardApplication ca,
			IccRecords ir, Context context, IccFileHandler fh, UiccCard ic) {
		if (ci == null || context == null) {
			throw new NullPointerException(
					"Service: Input parameters must not be null");
		}
	...
	/**
	 * This function validates the events in SETUP_EVENT_LIST which are
	 * currently supported by the Android framework. In case of SETUP_EVENT_LIST
	 * has NULL events or no events, all the events need to be reset.
	 */
	private boolean isSupportedSetupEventCommand(CatCmdMessage cmdMsg) {
		...
		for (int i = 0; i < cmdMsg.getSetEventList().eventList.length; i++) {
			...
			switch (eventval) {
			/*
			 * Currently android is supporting only the below events in
			 * SetupEventList Browser Termination, Idle Screen Available and
			 * Language Selection.
			 */
	...
	private void handleCommand(CommandParams cmdParams, boolean isProactiveCmd) {
		...
		case SET_UP_MENU:
			...
			resultCode = cmdParams.loadIconFailed ? ResultCode.PRFRMD_ICON_NOT_DISPLAYED
					: ResultCode.OK;
		...
		case DISPLAY_TEXT:
			...
			if (!cmdMsg.geTextMessage().responseNeeded) {
				resultCode = cmdParams.loadIconFailed ? ResultCode.PRFRMD_ICON_NOT_DISPLAYED
						: ResultCode.OK;
				sendTerminalResponse(cmdParams.cmdDet, resultCode, false, 0,
						null);
			}
			break;
		case REFRESH:
		...
		case LAUNCH_BROWSER:
			if ((((LaunchBrowserParams) cmdParams).confirmMsg.text != null)
					&& (((LaunchBrowserParams) cmdParams).confirmMsg.text
							.equals(STK_DEFAULT))) {
				message = mContext
						.getText(com.android.internal.R.string.launchBrowserDefault);
				((LaunchBrowserParams) cmdParams).confirmMsg.text = message
						.toString();
			}
			break;
		case SELECT_ITEM:
		...
		case SEND_USSD:
			if ((((DisplayTextParams) cmdParams).textMsg.text != null)
					&& (((DisplayTextParams) cmdParams).textMsg.text
							.equals(STK_DEFAULT))) {
				message = mContext
						.getText(com.android.internal.R.string.sending);
				((DisplayTextParams) cmdParams).textMsg.text = message
						.toString();
			}
			break;
		case PLAY_TONE:
		...
		case SET_UP_CALL:
			if ((((CallSetupParams) cmdParams).confirmMsg.text != null)
					&& (((CallSetupParams) cmdParams).confirmMsg.text
							.equals(STK_DEFAULT))) {
				message = mContext
						.getText(com.android.internal.R.string.SetupCallDefault);
				((CallSetupParams) cmdParams).confirmMsg.text = message
						.toString();
			}
			break;
		case OPEN_CHANNEL:
		...
		case SEND_DATA:
			...
			/*
			 * If the text mesg is null, need to send the response back to the
			 * card in the following scenarios - It has alpha ID tag with no
			 * Text Msg (or) - If alphaUsrCnf is not set. In the above cases
			...
			CatLog.d(this, "alphaUsrCnf: " + alphaUsrCnf + ", bHasAlphaId: "
					+ cmd.bHasAlphaId);
			...
			if ((cmd.textMsg.text == null) && (cmd.bHasAlphaId || !alphaUsrCnf)) {
				...
				if (isProactiveCmd) {
					sendTerminalResponse(cmdParams.cmdDet, ResultCode.OK,
							false, 0, null);
			...
			// Respond with permanent failure to avoid retry if STK app is not
			// present.
			if (!mStkAppInstalled) {
				CatLog.d(this, "No STK application found.");
				if (isProactiveCmd) {
					sendTerminalResponse(cmdParams.cmdDet,
							ResultCode.BEYOND_TERMINAL_CAPABILITY, false, 0,
							null);
					return;
				}
			}
			 ...
			 * either PROACTIVE_COMMAND or EVENT_NOTIFY. If PROACTIVE_COMMAND is
			 * used for those commands, send terminal response here.
			 */
			if (isProactiveCmd
					&& ((cmdParams.getCommandType() == CommandType.CLOSE_CHANNEL)
							|| (cmdParams.getCommandType() == CommandType.RECEIVE_DATA) || (cmdParams
							.getCommandType() == CommandType.SEND_DATA))) {
				sendTerminalResponse(cmdParams.cmdDet, ResultCode.OK, false, 0,
						null);
			}
			break;
	...
	private void encodeOptionalTags(CommandDetails cmdDet,
		...
		if (cmdType != null) {
			...
			case GET_INKEY:
				...
				if ((resultCode.value() == ResultCode.NO_RESPONSE_FROM_USER
						.value())
						&& (cmdInput != null)
			...
			case PROVIDE_LOCAL_INFORMATION:
				...
				break;
			default:
				CatLog.d(this, "encodeOptionalTags() Unsupported Cmd details="
						+ cmdDet);
				break;
			}
		} else {
	...
	private void eventDownload(int event, int sourceId, int destinationId,
		...
		/*
		 * Currently the below events are supported: Browser Termination, Idle
		 * Screen Available and Language Selection Event. Other event download
		...
		case IDLE_SCREEN_AVAILABLE_EVENT:
			CatLog.d(sInstance,
					" Sending Idle Screen Available event download to ICC");
			break;
		case LANGUAGE_SELECTION_EVENT:
			CatLog.d(sInstance,
					" Sending Language Selection event download to ICC");
	...
	public static CatService getInstance(CommandsInterface ci, Context context,
		...
		if (ic != null) {
			 ...
			 * application in itself - just get any FileHandler and IccRecords
			 * object
			 */
		...
		synchronized (sInstanceLock) {
			if (sInstance == null) {
				if (ci == null || ca == null || ir == null || context == null
						|| fh == null || ic == null) {
					return null;
				}
				HandlerThread thread = new HandlerThread(
						"Cat Telephony service");
				thread.start();
				...
				mIccRecords.registerForRecordsLoaded(sInstance,
						MSG_ID_ICC_RECORDS_LOADED, null);
				mUiccApplication.registerForReady(sInstance, MSG_ID_SIM_READY,
						null);
				CatLog.d(sInstance,
						"sr changed reinitialize and return current sInstance");
			}
			if (fh != null) {
				CatLog.d(sInstance,
						"Reinitialize the Service with new IccFilehandler");
	...
	public void handleMessage(Message msg) {
		...
		case MSG_ID_REFRESH:
			...
			mMsgDecoder.sendStartDecodingMessageParams(new RilMessage(msg.what,
					data));
			break;
		case MSG_ID_CALL_SETUP:
			mMsgDecoder.sendStartDecodingMessageParams(new RilMessage(msg.what,
	...
	private boolean validateResponse(CatResponseMessage resMsg) {
		...
		if ((resMsg.cmdDet.typeOfCommand == CommandType.SET_UP_EVENT_LIST
				.value())
				|| (resMsg.cmdDet.typeOfCommand == CommandType.SET_UP_MENU
						.value())) {
	...
	private void handleCmdResponse(CatResponseMessage resMsg) {
		...
		// non-interactive type Setup Event List and SETUP_MENU proactive
		// commands.
		// Stk framework would have already sent Terminal Response to Setup
		// Event
		// List and SETUP_MENU proactive commands. After sometime Stk app will
		// send
		// Envelope Command/Event Download. In which case, the response details
		// doesn't
		...
		case LAUNCH_BROWSER_ERROR:
			...
			case SET_UP_CALL:
				...
				// confirmation result is send back using a dedicated ril
				// message
			...
			case SET_UP_EVENT_LIST:
				if (IDLE_SCREEN_AVAILABLE_EVENT == resMsg.eventValue) {
					eventDownload(resMsg.eventValue, DEV_ID_DISPLAY,
							DEV_ID_UICC, resMsg.addedInfo, false);
				} else {
					eventDownload(resMsg.eventValue, DEV_ID_TERMINAL,
							DEV_ID_UICC, resMsg.addedInfo, false);
				}
		...
		case TERMINAL_CRNTLY_UNABLE_TO_PROCESS:
			// For screenbusy case there will be addtional information in the
			// terminal
			// response. And the value of the additional information byte is
			// 0x01.
		...
		sendTerminalResponse(cmdDet, resMsg.resCode,
				resMsg.includeAdditionalInfo, resMsg.additionalInfo, resp);
		mCurrntCmd = null;
	}
	...
	private boolean isStkAppInstalled() {
		...
		int numReceiver = broadcastReceivers == null ? 0 : broadcastReceivers
				.size();
