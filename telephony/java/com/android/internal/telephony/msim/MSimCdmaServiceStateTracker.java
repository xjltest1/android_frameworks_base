...
final class MSimCdmaServiceStateTracker extends CdmaServiceStateTracker {
	...
	protected UiccCardApplication getUiccCardApplication() {
		Subscription subscriptionData = ((MSimCDMAPhone) phone)
				.getSubscriptionInfo();
	...
	protected void updateSpnDisplay() {
		...
		if (!TextUtils.equals(plmn, mCurPlmn)) {
			...
			// would set showPlmn to true only if plmn was not empty, i.e. was
			// not
			// null and not blank. But this would cause us to incorrectly
			// display
			// "No Service". Now showPlmn is set to true for any non null
			// string.
	...
	protected void log(String s) {
		Log.d(LOG_TAG, "[MSimCdmaSST] [SUB : " + phone.getSubscription() + "] "
				+ s);
	}

	@Override
	protected void loge(String s) {
		Log.e(LOG_TAG, "[MSimCdmaSST] [SUB : " + phone.getSubscription() + "] "
				+ s);
	}
}
